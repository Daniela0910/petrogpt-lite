...
def render_sidebar():
    with st.sidebar:
        ...
        st.caption("Nota: Si cambias la API Key en los Secrets, reinicia la app.")
...
def tab_chat_tecnico():
    ...
    if prompt := st.chat_input("Consulta..."):
        ...
        with st.spinner("Consultando a PetroGPT..."):
            # AHORA SÍ: Usamos la función optimizada con caché
        ...
        if error:
            ...
            if st.button("🦮 Interpretar con IA"):
                with st.spinner("Analizando tendencia..."):
                    prompt = f"Analiza estos datos de Step Rate Test: {df.to_string()}. Identifica si hay un quiebre de pendiente."
                    st.info(get_ai_response_cached(prompt))
