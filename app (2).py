...
def get_ai_response_cached(prompt: str) -> str:
    """Llama a la IA y guarda el resultado en caché para evitar demoras innecesarias."""
    # Nota: El caché es útil para consultas idénticas o mantener la sesión del modelo viva
...
def render_sidebar():
    with st.sidebar:
        ...
        st.caption("Nota: Si cambias la API Key en los Secrets, reinicia la app para aplicar cambios.")
...
def tab_chat_tecnico():
    ...
    if prompt := st.chat_input("Consulta..."):
        ...
        with st.spinner("Consultando a PetroGPT..."):
            # Usamos la función con caché para mejorar la fluidez
        ...
        if error: 
            ...
            if st.button("🦮 Interpretar con IA"):
                with st.spinner("Analizando tendencia..."):
                    prompt = f"Analiza estos datos de Step Rate Test: {df.to_string()}. Identifica si hay un quiebre de pendiente y da una recomendación técnica."
